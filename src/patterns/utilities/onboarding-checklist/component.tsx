      ...
      {({ size }) => (
        <div className="utility-view u-split onboarding-view">
          <aside className="u-aside onboarding-aside">
            ...
            <div><span className="u-kicker">Seu progresso</span><div className="onboarding-ring" style={{ "--progress": `${progress * 3.6}deg` } as React.CSSProperties}><span><motion.b key={completed} initial={{ scale: 0.7 }} animate={{ scale: 1 }}>{completed}/{starterTasks.length}</motion.b><small>concluídos</small></span></div><p className="u-copy">Cada pequeno passo deixa seu espaço mais pronto para trabalhar.</p></div>
            <button type="button" className="onboarding-tour u-hide-small"><Play size={13} fill="currentColor" /> Ver tour de 90 segundos</button>
          </aside>

          <main className="u-main onboarding-main">
            <AnimatePresence mode="wait" initial={false}>
              {completed === starterTasks.length ? <motion.div key="celebration" className="onboarding-complete" initial={{ opacity: 0, scale: 0.86 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={utilitySpring}><motion.span animate={{ rotate: [0, -8, 8, 0], y: [0, -5, 0] }} transition={{ duration: 0.8 }}><PartyPopper size={29} /></motion.span><span className="u-kicker">Tudo pronto</span><h3>Seu espaço ganhou vida.</h3><p>Agora você já conhece o essencial. O restante pode ser descoberto no seu ritmo.</p><button type="button" className="u-primary" onClick={() => setDone(new Set([1]))}>Explorar novamente <ArrowRight size={14} /></button></motion.div> : <motion.div key="tasks" className="onboarding-content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.94 }}>
                <div className="u-row onboarding-head"><div><span className="u-kicker">Olá, Fabiano</span><b>Prepare seu espaço</b></div><span>{Math.round(progress)}%</span></div>
                <div className="u-progress onboarding-progress"><motion.span animate={{ width: `${progress}%` }} transition={utilitySpring} /></div>
                <motion.ul layout className="u-list onboarding-tasks"><AnimatePresence mode="popLayout" initial={false}>{starterTasks.slice(0, size === "small" ? 4 : size === "medium" ? 6 : 7).map((task) => { const Icon = task.icon; const checked = done.has(task.id); return <motion.li layout key={task.id} className={`u-list-item onboarding-task${checked ? " is-done" : ""}`} transition={utilitySpring}><button type="button" className="onboarding-check" onClick={() => toggle(task.id)} aria-pressed={checked}>{checked ? <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} transition={utilityQuickSpring}><Check size={12} /></motion.span> : <Circle size={15} />}</button><span className="u-list-icon"><Icon size={14} /></span><div className="u-grow"><b>{task.title}</b><small>{task.copy}</small></div><span className="onboarding-time">{task.minutes}</span><ArrowRight size={13} /></motion.li>; })}</AnimatePresence></motion.ul>
